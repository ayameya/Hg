...
**必ず先に [docs/hardware.md](docs/hardware.md) を読んでください。** ТГ1Б のデータシートの定格、
電源の作り方、2 つの入力方式の回路構成、レベル合わせの手順、乱数が本当に真空管から来ているかの確認方法を書いています。

| ТГ1Б の主な定格 | 値 |
| --- | --- |
| ヒーター | 6.3 V（5.7〜6.9 V）・225 ± 25 mA、予熱 10 秒以上 |
| 点弧電圧（グリッド 0 V） | 30 V 以下 |
| 放電中の電圧降下 | 20 V 以下 |
| 陽極電流 | 平均 20 mA、波高値 120 mA まで |
| 陽極電圧 | 順・逆方向とも波高値 240 V まで |
| グリッド回路の抵抗 | 10 kΩ〜1 MΩ |

グリッドを陰極と同電位にすれば 30 V 以下で点弧するので、陽極電源は 36〜48 V で足ります。
ヒーターの 6.3 V は USB の 5 V では足りないため、別に用意してください。
...
1. ヒーター（6.3 V）を点けて 10 秒以上予熱してから陽極電源を入れます（`TUBEPI_HV_ENABLE_PIN` を使えば自動化できます）。
